...
export default function Module6() {
  ...
  const [demoIndex, setDemoIndex] = useState(-1); // -1 = not started, tracks which move pair is being shown
  const [demoPlaying, setDemoPlaying] = useState(false);
  const [practiceIndex, setPracticeIndex] = useState(0); // which move pair the student is on
  ...
  const getMoveOptions = (sourceSquare: string) => {
    if (phase !== 'practice' || waitingForBlack || isLessonPass || !game) return;
    ...
    setDemoIndex(prev => {
      ...
      setGame(prevGame => {
        ...
        if (pair.black) {
          ...
          // No black move - demo is complete after this
          setDemoPlaying(false);
        }
  ...
  function onDrop(sourceSquare: string, targetSquare: string) {
    setOptionSquares({});
    if (phase !== 'practice' || waitingForBlack || isLessonPass) return false;
    ...
    try {
      ...
      if (moveResult) {
        if (moveResult.san === currentPair.white) {
          // Correct move!
          setGame(g);
          ...
          if (currentPair.black) {
            ...
            setTimeout(() => {
              setGame(prevG => {
                ...
                try {
                  const bResult = g2.move(currentPair.black!);
                  if (bResult) {
                    playMoveSound();
                    setMoveHistory(h => [...h, bResult.san]);
                  }
            ...
            // No black reply = this was the last move
            setFeedback({ type: 'success', message: lesson.successMessage });
            setIsLessonPass(true);
            markLessonCompleted(lesson.id);
          }
          ...
          // Wrong move
          setFeedback({ type: 'error', message: `Movimiento incorrecto. Debes jugar ${currentPair.white}. ${currentPair.hint || ''}` });
          return false;
        }
      }
    } catch (_e) {
    ...
    <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 p-6 md:p-8 flex flex-col md:flex-row gap-8 min-h-[600px] transition-colors duration-300">
      ...
      <div className="flex-1 flex flex-col gap-5">
        <div>
          ...
          <h2 className="text-3xl font-bold text-slate-900 dark:text-slate-100 mt-1 mb-3">{lesson.title}</h2>
          ...
          {phase === 'demo' ? (
            <div className="bg-amber-50 dark:bg-amber-900/20 border-l-4 border-amber-500 p-4 rounded-r-lg">
              ...
              <p className="text-amber-700 dark:text-amber-300 m-0 font-medium text-sm">
                Observa cómo se desarrolla la apertura en vivo. Las piezas se mueven automáticamente para que entiendas la secuencia.
              </p>
              {!demoPlaying && (
                <button
                  onClick={startDemo}
            ...
            <div className="bg-emerald-50 dark:bg-emerald-900/20 border-l-4 border-emerald-500 p-4 rounded-r-lg">
              ...
              {!isLessonPass && lesson.moves[practiceIndex] && (
                <div className="mt-3 bg-white/70 dark:bg-slate-800/70 rounded-lg px-3 py-2 text-sm text-emerald-800 dark:text-emerald-200 font-medium flex items-center gap-2">
                  💡 Pista: {lesson.moves[practiceIndex].hint}
                </div>
              )}
              {waitingForBlack && (
        ...
        <div className="mt-auto pt-6 border-t border-slate-100 dark:border-slate-700 flex justify-between items-center">
          ...
          {currentLessonIndex < module6Lessons.length - 1 ? (
            <button
              onClick={() => nextLesson(module6Lessons.length)}
      ...
      <div className="w-full md:w-[450px] lg:w-[500px] xl:w-[600px] flex flex-col gap-4">
        <div className={`bg-slate-50 dark:bg-slate-900/50 p-2 md:p-4 rounded-2xl border transition-colors shadow-inner relative ${
          ...
          {/* Overlay when demo is playing or waiting for black */}
          {(phase === 'demo' || waitingForBlack) && (
          ...
          <Chessboard
            ...
            arePiecesDraggable={phase === 'practice' && !waitingForBlack && !isLessonPass}
            customBoardStyle={{
              borderRadius: '8px',
        ...
        <div className="flex justify-between items-center px-2">
          <span className="text-sm font-medium text-slate-500 dark:text-slate-400">
            {phase === 'demo' ? '🎬 Demostración' : waitingForBlack ? '⬛ Turno del oponente' : '⬜ Tu turno'}
          </span>
          <button
            onClick={() => {
              if (demoTimerRef.current) clearTimeout(demoTimerRef.current);
